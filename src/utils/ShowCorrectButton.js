...
function ShowCorrectButton(props) {
  ...
  const buyTickets = () => {
    if (props.correctAnswer) {
      ...
      setError({
        visible: "visible",
        message: "WRONG SECURITY QUESTION ANSWER",
      });
      props.regenerateQuestion();
      setTimeout(() => {
        setError({ visible: "hidden", message: "hidden" });
  ...
  if (context.user._id === undefined) {
    return (
      <Link to="/log" css={{ width: "100%" }}>
        <button css={{ width: "100%", margin: "1rem 0" }} className="button02">
          LOG IN TO PURCHASE
        </button>
      </Link>
    );
    ...
    return (
      <React.Fragment>
        <button
          css={{ width: "100%", margin: "1rem 0", cursor: "not-allowed" }}
          className="button02"
        >
